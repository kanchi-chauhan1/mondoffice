(function () {
    function addCouponCode() {
        ...
        document.querySelector('.custom-input-code').addEventListener('input',(event)=>{
            let placeholderHeading = document.querySelector('.custom-placeholder-heading');
            let customMessage = document.querySelector('.custom-message');
            let submitButton = document.querySelector('.custom-promocode-submit');
            if (event.target.value === "") {
                placeholderHeading.classList.remove('failure');
                placeholderHeading.classList.remove('success');
                placeholderHeading.classList.remove('show-heading');
                placeholderHeading.innerHTML="";
                customMessage.innerHTML="";
                customMessage.classList.remove('success');
                customMessage.classList.remove('failure');
                ...
                submitButton.innerHTML=`<svg width="19" class="arrow" "height="11" viewBox="0 0 19 11" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    function showMessage() {
        document.querySelector('.custom-promocode-submit').addEventListener('click',function () {
            ...
            if (document.querySelector('.custom-input-code').value === "") {
                ...
                placeholderHeading.classList.remove('failure');
                placeholderHeading.classList.add('show-heading');
                placeholderHeading.classList.add('success');
                customMessage.innerText="Lo sconto è stato applicato.";
                customMessage.classList.add('success');
                document.querySelector('.custom-input-code').style.borderBottom="2px solid green";
                submitButton.innerHTML=`<svg width="16" height="11" viewBox="0 0 16 11" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                placeholderHeading.classList.add('show-heading');
                placeholderHeading.classList.remove('success');
                placeholderHeading.classList.add('failure');
                customMessage.innerText="Questo codice promozionale non è valido.";
                customMessage.classList.add('failure');
                customMessage.classList.remove('success');
                document.querySelector('.custom-input-code').style.borderBottom="2px solid red";
                submitButton.innerHTML=`<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
